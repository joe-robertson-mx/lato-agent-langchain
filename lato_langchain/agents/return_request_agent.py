...
# This module was copied from the in-repo langchain package and adapted
# to live under the `lato_langchain` namespace to avoid name collision
# with the external `langchain` library. It still imports the external
# `langchain.agents.create_agent` at runtime when needed.
...
def create_return_request_agent(model: str = "openai:gpt-5-mini"):
    ...
    Note: `create_agent` is provided by the external `langchain` library.
    """
    # import the actual create_agent function at runtime to avoid import cycles
...
def run_return_request_agent(
    ...
    summary_text = None
    try:
        # Lazy import of helper from this package
        from lato_langchain.utils.print_helpers import find_repair_summary
